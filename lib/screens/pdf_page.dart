...
Future<File> generatePdf({
                      ...
                      pw.Column(
                        ...
                        children: [
                          ...
                          pw.Row(
                              ...
                              children: [
                                pw.Text(
                                    "Nº DO ORÇAMENTO - $generatedNOrcamento"),
                              ]),
                          pw.SizedBox(height: 16),
                          pw.Center(
                              heightFactor: 2,
                              child: pw.Text('Informações do Cliente',
                                  style: pw.TextStyle(
                                      fontWeight: pw.FontWeight.bold))),
                          pw.SizedBox(height: 16),
                          pw.Row(
                          ...
                          pw.Center(
                              heightFactor: 2,
                              child: pw.Text('Informações do Carro',
                                  style: pw.TextStyle(
                                      fontWeight: pw.FontWeight.bold))),
                          pw.SizedBox(height: 16),
                          pw.Row(
                          ...
                          pw.Center(
                            heightFactor: 2,
                            child: pw.Text('Itens do Orçamento:',
                                style: pw.TextStyle(
                                    fontWeight: pw.FontWeight.bold)),
                          ),
  ...
  final output = Directory('/storage/emulated/0/Documents/Orcamentos');
  if (!output.existsSync()) {
    await output.create(recursive: true);
  }

  // Criar uma subpasta chamada "Orcamentos"
  final folderPath = '${output.path}/Orcamentos';
  final folder = Directory(folderPath);

  if (!folder.existsSync()) {
    await folder.create(recursive: true); // Cria a pasta se não existir
  }

  // Caminho completo do arquivo
  final filePath = '$folderPath/orc-$generatedNOrcamento.pdf';
...
void notifyMediaScanner(String filePath) {
  try {
    final methodChannel =
        const MethodChannel('com.codeexperts.orcamento_app/media');
    methodChannel.invokeMethod('scanFile', {"path": filePath});
  } catch (e) {
    print('Erro ao notificar o Media Scanner: $e');
  }
}
